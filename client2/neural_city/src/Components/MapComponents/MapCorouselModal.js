...
function MapCorouselModal({ ward_id, ward_name, handleCorouselClose, handleCorouselOpen, corouselOpen }) {
    ...
    useEffect(() => {
        (async function fetch_data() {
            setLaoding(true);
            try {
                const res = await api.get(`/workflows/?ward_id=${ward_id}`);
                console.log(res);
                console.log(ward_id);
                setData(res.data.workflows);
            } catch (err) {
            ...
            >
                {loading ? <div>Loading...</div> : error ? <div>{error}</div> :
                    <div className='z-[10000] bg-white pb-0 shadow-lg sm:w-[65%] w-[98%] rounded-lg absolute top-[5%] left-[20%]'>
                        ...
                        <hr />
                        <div className='p-2'>
                            <CourouselComponentMap datapoints={data} />
                        </div>
                    </div>}
            </Modal>
        </div>
